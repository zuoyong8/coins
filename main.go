...
func main(){
	...
	auth.Use(authMiddleware.MiddlewareFunc())
	{
		...
		//ethereum
		auth.GET("/ethereum/web3sha3", controllers.Web3Sha3)
		auth.GET("/ethereum/ethcall", controllers.EthCall)
		auth.GET("/ethereum/web3clientversion", controllers.Web3ClientVersion)
		auth.GET("/ethereum/netversion", controllers.NetVersion)
		auth.GET("/ethereum/getsyning", controllers.GetSyning)
		auth.GET("/ethereum/getaccouts", controllers.GetAccouts)
		auth.GET("/ethereum/getblocknumber",controllers.GetBlockNumber)
		auth.GET("/ethereum/getcoinbaseaddress", controllers.GetCoinbaseAddress)
		auth.GET("/ethereum/personalnewaccount", controllers.PersonalNewAccount)
		auth.GET("/ethereum/personallistwallets", controllers.PersonalListWallets)
